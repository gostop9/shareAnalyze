...
void main()
{
	...
	// 竞价大成交大垫单
	vector<PROPERTY_t> jingJiaBigMeetVector;
	jingJiaBigMeetVector.reserve(vectorSize);
	...
	char dataPre2JingJiaFileName[100];
	memset(date, 0, sizeof(date));
	memset(path, 0, sizeof(path));
	memset(dataPre, 0, sizeof(dataPre));
	memset(data2Pre, 0, sizeof(data2Pre));
	memset(dataPreJingJiaFileName, 0, sizeof(dataPreJingJiaFileName));
	memset(dataPre2JingJiaFileName, 0, sizeof(dataPre2JingJiaFileName));
	...
	ExcelRwC excelReadWrite(ztfpFlag);
	...
	vector<ZHANGFU_t> pre2JingJiazhangfu;
	...
	//if(1 >= fileIndex)
	{
		...
		//增加前2日竞价文件读取，比较连板股的竞价金额
		strcpy(dataPre2JingJiaFileName, path);
		strcat(dataPre2JingJiaFileName, "zhangfu_");
		strcat(dataPre2JingJiaFileName, data2Pre);
		strcat(dataPre2JingJiaFileName, ".txt");
		...
		if (cnt_int > 20180525)
		{
			...
			readZhangfuFile(dataPre2JingJiaFileName, pre2JingJiazhangfu);
		}
	...
	for (int iFile = fileIndex; iFile >= 0; iFile--)
	{
		...
		// 提取前2交易日的开盘价和收盘价
		if (0 < pre2JingJiazhangfu.size())
		{
			vector<PROPERTY_t>::iterator proIter = propertyV[vecIndex].begin();
			while (proIter != propertyV[vecIndex].end())
			{
				vector<ZHANGFU_t>::iterator zfIter = pre2JingJiazhangfu.begin();
				while (zfIter != pre2JingJiazhangfu.end())
				{
					char codeTemp[10];
					memset(codeTemp, 0, sizeof(codeTemp));

					if (6 == strlen(zfIter->code))
					{
					...
					if (0 == strcmp(codeTemp, zfIter->code))
					{
						proIter->pre2XianJia = zfIter->xianJia;
						proIter->pre2KaiPan = zfIter->kaiPan;
						break;
					}
					++zfIter;
				}
		...
		//if (1 >= fileIndex)
		{
			...
			// 用于去重
			int vectorSize = propertyAnalyVecBlock.size();
			vector<PROPERTY_t> propertyAnalyVecBlockDupRemove;
			propertyAnalyVecBlockDupRemove.reserve(vectorSize);
			propertyAnalyVecBlockDupRemove.clear();

			PROPERTY_t analyProtyExist = propertyAnalyVecBlock[0];
			propertyAnalyVecBlockDupRemove.push_back(analyProtyExist);
			for (int analyIdx = 1; analyIdx < vectorSize; analyIdx++)
			{
				PROPERTY_t analyProty = propertyAnalyVecBlock[analyIdx];
				if (strcmp(analyProty.code, analyProtyExist.code))
				{
					propertyAnalyVecBlockDupRemove.push_back(analyProty);
				}
				analyProtyExist = analyProty;
			}

			// 提取昨日涨停,今日竞价符合预期股票
			for (int analyIdx = 0; analyIdx < propertyAnalyVecBlockDupRemove.size(); analyIdx++)
			{
				PROPERTY_t analyProty = propertyAnalyVecBlockDupRemove[analyIdx];
				bool jinRiYiZiBanFlag = yiZiBanJudge(analyProty);
				bool bigDeal_and_Diandan = (analyProty.dianDanJinE > dianDanBigThreshold) && (jingJiaJinEThreshould < analyProty.zongJinE) && (analyProty.weiBi < weiBiMaxThresholdBigDeal);
				bool erban8_2to3 = (analyProty.continueDay == 2) && ((analyProty.zuoRiKaiPanZhangFu < 8.99) && (analyProty.zuoRiKaiPanZhangFu > 7.99)) && (!jinRiYiZiBanFlag);
				string indCode = analyProty.code;
				// N字板判断，三天前涨停
				bool zhangTing3PreFlag = false;
				zhangTing_t zhangTing3Pre;
				// 前交易日一字板判断
				for (int zt3PreIdx = 0; zt3PreIdx < zhangTingVec3Pre.size(); zt3PreIdx++)
				{
					...
					if (index != string::npos)
					{
						...
						zhangTing3Pre = zhangTingVec3Pre[zt3PreIdx];
						break;
					}
				}
				...
				{
					// 2pre 收涨
					//if (zhangTing3Pre.maxPrice * 1.1 > analyProty.zuoRiZuiGao)
					if (analyProty.pre2XianJia < analyProty.pre2KaiPan)
					{
						continue;
					}
				}
				...
				bool zhangTingJia_bigWeibi = ((analyProty.zhangFu > kaiPanZfMaxLimit) && (analyProty.weiBi > weiBiMaxThreshold) && (analyProty.dianDanJinE > gaoWeiBidianDanThreshold + analyProty.continueDay * 100.0) && (analyProty.zongJinE < analyProty.dianDanJinE * TENTHOUSAND * 6.0));
				...
				// 昨日一字板，成交大于涨停封单，今日继续一字板
				//if (zuoRiyiZiBan_Flag && jinRiYiZiBanFlag && (analyProty.zuoRiZongJinE > analyProty.limitUpMoney) && (analyProty.continueDay == 2) && (analyProty.dianDanJinE < 4500.0))
				if (zuoRiyiZiBan_Flag && jinRiYiZiBanFlag && (analyProty.zuoRiZongJinE > analyProty.limitUpMoney) && (analyProty.continueDay == 2) && (analyProty.zuoRiZongJinE > 280.0 * ONE_MILLION))
				{
					continue;
				}
				// 昨日一字板，成交大于涨停封单
				if (zuoRiyiZiBan_Flag && (analyProty.zuoRiZongJinE > analyProty.limitUpMoney) && (analyProty.zuoRiZongJinE > 280.0 * ONE_MILLION) && (analyProty.dianDanJinE < 10000.0))
				{
					continue;
				}

				// 昨日T字板，振幅小于10，不能下水
				if (zuoRiyiZiBanTFlag && (analyProty.zuoRiZhenFu > 10.5))
				{
					continue;
				}
				...
				// 昨日竞价金额大于50万
				if (10.0 * TENTHOUSAND > analyProty.preJingJiaZongJinE)
				{
					continue;
				}
				...
				for (int ztPreIdx = 0; ztPreIdx < zhangTingVecPre.size(); ztPreIdx++)
				{
						...
						//if (!strcmp(zhangTingVecPre[ztPreIdx].code, indCode.c_str()))
					{
						...
						if ((jingJiaJinE_flag) &&
							...
							((analyProty.preJingJiaZongJinE - jingJiaJinEChaThreshould * TENTHOUSAND < analyProty.zongJinE) || ((jingJiaJinEChaRatio < analyProty.zongJinE / analyProty.preJingJiaZongJinE) && (analyProty.zongJinE > 5000.0 * TENTHOUSAND))) &&
			...
			// 打印竞价放量且垫单大的股票
			{
				int shareNum = propertyAnalyVecSort.size();
				for (int analyIdx = 0; analyIdx < shareNum; analyIdx++)
				{
					PROPERTY_t analyProty = propertyAnalyVecSort[analyIdx];
					if ((10000.0 * TENTHOUSAND < analyProty.zongJinE) 
						&& (analyProty.dianDanJinE > 6249.0) 
						&& ((analyProty.continueDay > 0) 
							|| (analyProty.zhangFu > 4.99) 
							|| ((analyProty.zhangFu > 3.99) && (analyProty.ziYouLiuTongShiZhi > 100 * DIVIDE))))
					{
						jingJiaBigMeetVector.push_back(analyProty);
					}
				}
				// 赋值在其他板块非第一的flag
				...
				{
					vector<PROPERTY_t>::iterator it = jingJiaBigMeetVector.begin();
					while (it != jingJiaBigMeetVector.end())
					{
				...
				std::sort(jingJiaBigMeetVector.begin(), jingJiaBigMeetVector.end(), cmpMethod_zhangFuIndex);
			}
		...
		if (1 == fileIndex)
		{
			...
			//插入标题
			excelReadWrite.excelRowIndex++;
			char excelHeaderSymbol = excelReadWrite.excelWriteHeaderC(excelReadWrite.rgMyRge);

			// 标记符合条件中竞价最大成交个股
			float max_deal_jingjia = 0;
			int maxDeal_index = 0;
			for (int analyIdx = 0; analyIdx < resultVecZtPreMeet.size(); analyIdx++)
			{
				PROPERTY_t analyProty = resultVecZtPreMeet[analyIdx];
			...
			resultVecZtPreMeet[maxDeal_index].maxJingJiaDealFlag = true;
			...
			// 显示竞价放量且垫单大的股票
			excelReadWrite.excelRowIndex++;
			excelReadWrite.writeExcelSheet(jingJiaBigMeetVector, flag_guXinZhengShu);
	...
	if (1 == fileIndex)
	{
		...
		if (0 != strcmp(subName.c_str(), "result_zt"))
		{
			...
			tdxBlockModify(zxgTdxFileName, resultZxgSet);
